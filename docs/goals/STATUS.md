...
| Goal 3：文档生成 | 部分完成 | 三个真实 AI 样例、Markdown、截图、预览和审核门禁已实现；线索转化截图与人工审核待补充 |
| Goal 4：飞书发布 | 部分完成 | 修订 8 已首次真实发布，远端 22 个 Block 与本地 completed 审计通过；待重复发布和人工排版验收 |
| Goal 5：真实试点 | 进行中 | 3 个试点均完成真实 AI 样例；两个新增试点完成 AI/截图整合与本地预览，逐条人工审核、线索转化截图及重复发布仍待完成 |
...
- 真实 CRM“线索转化”已形成批准并首次发布的修订 8，以及新的真实 AI 待审修订 9；项目发起评审和角色维护已形成 AI 内容、业务标题和脱敏截图整合修订 8，仍为 `needs_review`；另有 2 个未选候选修订。
...
- `pnpm check`：2026-08-23 最新完整运行中，16 个测试文件、63 个测试及全部质量门禁通过。
